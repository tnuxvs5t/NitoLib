...
int main() {
    ...
    int inverse_brute = -1;
    for (int candidate = 0; candidate < 97; ++candidate)
    ...
    int crt_brute = -1;
    for (int value = 0; value < 97 * 89; ++value)
    ...
    for (int round = 0; round < 2000; ++round) {
        ...
        if (rng() & 1) a = -a;
        if (rng() & 1) b = -b;
        int modulus_a = 1 + int(rng() % 40), modulus_b = 1 + int(rng() % 40);
        auto result = ncrt(a, modulus_a, b, modulus_b);
        int residue_a = int(a % modulus_a), residue_b = int(b % modulus_b);
