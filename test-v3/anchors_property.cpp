...
struct anchor_hash {
    ...
    size_t operator()(const anchor_key& key) const {
        ++*calls;
...
struct anchor_equal {
    ...
    bool operator()(const anchor_key& a, const anchor_key& b) const {
        ++*calls;
...
int main() {
    ...
    for (int round = 0; round < 5000; ++round) {
        ...
        auto function = nanchors(nall(keys), nall(values),
                                 anchor_hash{&hash_calls}, anchor_equal{&equal_calls});
        CHECK(function.len() == n);
        for (int i = 0; i < n; ++i) {
            anchor_key copied = keys[i];
