...
int main() {
    vector<int> values{1, 1, 2, 2, 2, 3, 1, 1};
    auto blocks = nblocks(nall(values), 3);
    CHECK(blocks.len() == 3);
    CHECK(blocks.key(0) == pair(0, 3) && blocks.key(2) == pair(6, 8));
    CHECK((ncollect(blocks[0]) == vector<int>{1, 1, 2}));
    CHECK((ncollect(blocks(pair{3, 6})) == vector<int>{2, 2, 3}));
    auto detached = nblocks(nall(values), 3)[2];
    detached[0] = 7;
    CHECK(values[6] == 7 && ncollect(nblock(nall(values), 3, 2)) == vector<int>({7, 1}));

    auto windows = nwindows(nall(values), 3, 2);
    CHECK(windows.len() == 3);
    CHECK(windows.key(1) == pair(2, 5));
    CHECK((ncollect(windows[1]) == vector<int>{2, 2, 2}));
    CHECK(nwindows(nall(values), 20).len() == 0);

    vector<int> semantic_keys{80, 10, 70, 20, 60, 30, 50, 40};
    array<int, 81> locate{};
    for (int i = 0; i < int(semantic_keys.size()); ++i) locate[semantic_keys[i]] = i;
    ...
    for (int round = 0; round < 15000; ++round) {
        int n = int(rng() % 55);
        vector<int> input(n);
        for (int& value : input) value = int(rng() % 8);

        int width = 1 + int(rng() % 12);
        auto partition = nblocks(nall(input), width);
        int expected_blocks = n / width + (n % width != 0);
        CHECK(partition.len() == expected_blocks);
        for (int block = 0; block < partition.len(); ++block) {
            int left = block * width, right = min(n, left + width);
            CHECK(partition.key(block) == pair(left, right));
            auto chunk = partition[block];
            CHECK(chunk.len() == right - left);
            for (int i = left; i < right; ++i) CHECK(chunk[i - left] == input[i]);
        }

        int window_width = 1 + int(rng() % 12), step = 1 + int(rng() % 8);
        auto window_list = nwindows(nall(input), window_width, step);
        int expected_windows = window_width <= n ? 1 + (n - window_width) / step : 0;
        CHECK(window_list.len() == expected_windows);
        for (int window = 0; window < window_list.len(); ++window) {
            int left = window * step, right = left + window_width;
            CHECK(window_list.key(window) == pair(left, right));
            for (int i = left; i < right; ++i) CHECK(window_list[window][i - left] == input[i]);
        }

        vector<pair<int, int>> expected_runs;
        int left = 0;
        for (int i = 1; i <= n; ++i)
            if (i == n || input[i - 1] != input[i])
                expected_runs.push_back({left, i}), left = i;
        auto runs = nruns(nall(input));
        CHECK(runs.len() == int(expected_runs.size()));
        for (int run = 0; run < runs.len(); ++run) {
            CHECK(runs.key(run) == expected_runs[run]);
            auto [begin, end] = expected_runs[run];
            auto chunk = runs[run];
            CHECK(chunk.len() == end - begin);
            for (int i = begin; i < end; ++i) CHECK(chunk[i - begin] == input[i]);
        }

        int threshold = int(rng() % 4);
        auto tolerant = nruns(nall(input), [=](int left_value, int right_value) {
            return abs(left_value - right_value) <= threshold;
        });
        int count = n ? 1 : 0;
        for (int i = 1; i < n; ++i) count += abs(input[i - 1] - input[i]) > threshold;
