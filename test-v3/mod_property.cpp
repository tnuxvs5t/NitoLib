...
constexpr i128 power10(int exponent) {
    i128 value = 1;
...
int main() {
    ...
    static_assert(int(mint(huge)) == 2401);
    static_assert(int(mint(-huge)) == 999'997'606);
    ...
    static_assert(int(mint(2).pow(huge)) == 358'860'370);
    ...
    for (int modulus = 1; modulus <= 200; ++modulus) {
        for (long long value = -1000; value <= 1000; ++value) {
            int expected = int(value % modulus);
        ...
        for (int left = 0; left < modulus; ++left)
            for (int right = 0; right < modulus; ++right) {
                ...
                CHECK(nmod_mul(left, right, modulus) == int(1LL * left * right % modulus));
    ...
    for (int round = 0; round < 100000; ++round) {
        int modulus = int(rng() % INT_MAX) + 1;
        u128 unsigned_value = u128(rng()) << 64 | rng();
        int unsigned_expected = int(unsigned_value % unsigned(modulus));
        ...
        CHECK(int(mint(signed_value)) == int(mint_remainder));
    }
}
