...
struct nmatrix {
    ...
    nmatrix() = default;
    explicit nmatrix(int row_count, int column_count, const T& value = T{})
        : rows(row_count), columns(column_count), data(size_t(rows) * columns, value) {}

    int len() const { return rows; }
    T& operator()(int row, int column) { return data[size_t(row) * columns + column]; }
    const T& operator()(int row, int column) const { return data[size_t(row) * columns + column]; }
    auto operator[](int row) { return nsub(nall(data), row * columns, (row + 1) * columns); }
    auto operator[](int row) const { return nsub(nall(data), row * columns, (row + 1) * columns); }

    static nmatrix identity(int size) {
...
struct nrref_result {
    nmatrix<T> matrix;
    vector<int> pivot;
    int rank() const { return int(pivot.size()); }
};
...
*/
template <class T>
nrref_result<T> nrref(nmatrix<T> matrix, int pivot_columns = -1) {
    if (pivot_columns < 0) pivot_columns = matrix.columns;
    vector<int> pivot;
    int next_row = 0;
    for (int column = 0; column < pivot_columns && next_row < matrix.rows; ++column) {
        int chosen = next_row;
        while (chosen < matrix.rows && matrix(chosen, column) == T{}) ++chosen;
        if (chosen == matrix.rows) continue;
        for (int c = 0; c < matrix.columns; ++c) swap(matrix(next_row, c), matrix(chosen, c));
        T scale = T(1) / matrix(next_row, column);
        for (int c = 0; c < matrix.columns; ++c) matrix(next_row, c) *= scale;
        for (int row = 0; row < matrix.rows; ++row) if (row != next_row) {
...
T ndeterminant(nmatrix<T> matrix) {
    ...
    for (int column = 0; column < matrix.rows; ++column) {
        ...
        if (chosen != column) {
            for (int c = column; c < matrix.columns; ++c)
        ...
        for (int row = column + 1; row < matrix.rows; ++row) {
            ...
            for (int c = column + 1; c < matrix.columns; ++c)
...
nlinear_solution<T> nlinear_solve(nmatrix<T> coefficients, V right) {
    int equations = coefficients.rows, variables = coefficients.columns;
    ...
    for (int row = reduced.rank(); row < equations; ++row)
    ...
    for (int row = 0; row < reduced.rank(); ++row) {
        int column = reduced.pivot[row];
        is_pivot[column] = true;
        particular[column] = reduced.matrix(row, variables);
    }
    vector<vector<T>> basis;
    for (int free = 0; free < variables; ++free) if (!is_pivot[free]) {
