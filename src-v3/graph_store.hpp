...
struct ncsr {
    ...
    ncsr(int n, V edges, From from, To target)
        : vertices(nrange(n)), offset(n + 1), to(move(target)) {
        for (int i = 0; i < edges.len(); ++i) ++offset[invoke(from, edges[i]) + 1];
        ...
        for (int i = 0; i < edges.len(); ++i) {
            int source = invoke(from, edges[i]);
    ...
    auto view() {
        return ngraph{vertices, [this](int vertex) { return edges(vertex); },
...
auto nmake_csr(int vertices, V edges, From from, To to) {
